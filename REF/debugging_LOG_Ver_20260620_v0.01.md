...
## 🐛 이슈 #12 — DioException receive timeout: 구버전 APK 타임아웃 불일치

### 증상
```
요약 생성 실패: DioException [receive timeout]: The request took longer than 0:03:00.000000 to receive data.
```
앱이 3분 정확히 기다리다가 타임아웃. 동일 에러 두 번 반복.

### 원인
코드상 `_receiveTimeout = Duration(seconds: 360)`으로 수정했음에도 설치된 APK는 180초 타임아웃.
**APK를 재빌드하지 않아 구버전(180s)이 그대로 설치된 상태**였음.
...
// 설치된 APK는 이전 빌드(180s)
// → 소스 수정 후 반드시 재빌드 + 재설치 필요
```

추가 원인: Gemini의 YouTube 영상 분석 + Claude 요약 = 4~5분 영상 기준 3분+ 소요 가능.

### 해결
```powershell
# frontend 디렉토리에서
flutter build apk --release   # 21.1MB, 약 9분 소요
adb install -r "build\app\outputs\flutter-apk\app-release.apk"
```

### 교훈
- 코드 수정 후 반드시 APK 재빌드 + 재설치 확인 (소스 변경 ≠ 설치된 앱 변경)
- Gemini 처리 시간은 영상 길이에 비례 → 360초(6분) 타임아웃도 긴 영상엔 부족할 수 있음
- 테스트는 5분 이하 짧은 영상으로 먼저 진행

---

## 🐛 이슈 #13 — type 'Null' is not a subtype of type 'List\<dynamic\>' in type cast

### 증상
```
요약 생성 실패: type 'Null' is not a subtype of type 'List<dynamic>' in type cast
```
서버 응답은 성공(HTTP 200)했지만 Flutter JSON 파싱 단계에서 크래시.

### 원인
FastAPI/Pydantic의 기본 JSON 직렬화는 **snake_case** (`key_insights`, `processing_time_ms`).  
Flutter `@JsonSerializable()` 생성 코드는 **camelCase** (`keyInsights`, `processingTimeMs`) 키를 기대.

```
백엔드 응답:  { "key_insights": [...], "processing_time_ms": 1234 }
Flutter 파싱: json['keyInsights']  → null
              (json['keyInsights'] as List<dynamic>)  → 타입 캐스트 에러
```

`categories`, `stockRelated`, `emailSent`, `githubPagesUpdated` 등 모든 snake_case 필드 동일 문제.

### 해결
`backend/app/models.py`에 CamelCase 베이스 모델 추가:
```python
...
`summarize_router.py` 라우터에 alias 직렬화 활성화:
```python
@router.post("/summarize", response_model=FullResult, response_model_by_alias=True)
```

### 교훈
- FastAPI + Flutter 연동 시 snake_case ↔ camelCase 불일치 반드시 사전 확인
- Pydantic v2는 `ConfigDict(alias_generator=to_camel)` + `response_model_by_alias=True` 조합으로 해결
- 에러 메시지가 "Null is not subtype of List"면 JSON 키 이름 불일치가 첫 번째 의심 대상
...
| 2026-06-21 | v0.01 | 구버전 APK 타임아웃 불일치(180s) → 재빌드 후 360s 적용 (이슈 #12) |
| 2026-06-21 | v0.01 | FastAPI snake_case ↔ Flutter camelCase 불일치 → CamelModel 베이스 추가 (이슈 #13) |

---

*다음 이슈 발생 시 이 파일에 ## 이슈 #14 로 추가*
